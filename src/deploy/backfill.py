...
Usage:
    python -m src.deploy.backfill --days 8
"""
...
def backfill(days: int = 8) -> None:
    ...
    for forecast_date in target_dates:
        ...
        try:
            forecast_hourly, model_predictions = _predict_for_date(
                df_trunc, models, weights, config, groups
            )
...
def _predict_for_date(
    ...
    # Per-model predictions (matches inference.py format)
    model_predictions = {}
    ...
    return forecast_hourly, model_predictions
